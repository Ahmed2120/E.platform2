...
class CoursesAndGroupsPage extends StatefulWidget {
   CoursesAndGroupsPage({required this.model, this.flag = 0,Key? key}) : super(key: key);
   MainModel model;
   int flag;
  @override
  State<CoursesAndGroupsPage> createState() => _CoursesAndGroupsPageState();
}

class _CoursesAndGroupsPageState extends State<CoursesAndGroupsPage> {
  ...
  //   widget.model.fetchgroups(widget.model.allSubjects[0].Id, '');
    ...
    if(widget.flag == 1) type = ContentType.group;
    super.initState();
  }
            ...
            child: Stack(
                ...
                //  height: deviceSize.height,
                  ...
                  decoration: const BoxDecoration(
                     ...
                     //   SizedBox(height: deviceSize.height * 0.05,),
                        SizedBox(
                          ...
                          : ListView.builder(
                            ...
                            itemBuilder: (context, index)=> subjectContainer(model.allSubjects[index]),
                          ),
                        ),
                        // Container(
                        ...
                        Row(
                          ...
                          children: [
                            GestureDetector(onTap: () =>
                                ...
                                }),child: Text('كورسات', style: TextStyle(fontSize: 17, color: typeColor(ContentType.course)),)),
                            GestureDetector(onTap: () =>
                                setState(() {
                                  type = ContentType.group;
                                }),child: Text('مجموعات', style: TextStyle(fontSize: 17, color: typeColor(ContentType.group)),)),
                          ],
                        ),
                        const SizedBox(height: 10,),
                        // ---------------------- courses ------------------
                        if (type == ContentType.course)
                          ...
                          const Center(child: CircularProgressIndicator())
                             ...
                             child: GridView.builder(
                               ...
                               itemBuilder: (context, index)=> courseContainer(context,model.allCourses[index], onStart: ()=> GlobalMethods.navigate(context,  ScopedModelDescendant<MainModel>(
                                   builder:(context,child,MainModel model){
                                     return CourseContentPage(model: model,
                                         course:model.allCourses[index]); }),
                               ),),
                             ),
                           )
                        // ---------------------- groups ------------------
                        else model.groupLoading==true?
                        const Center(child: CircularProgressIndicator())
                            :Expanded(
                               child: GridView.builder(
                                 shrinkWrap: true,
                                ...
                                childAspectRatio: 0.8
                                 ...
                                 itemCount: model.allGroups.length,
                                  itemBuilder: (context, index)=> groupContainer(context,model.allGroups[index], onStart: ()=>
                                        GlobalMethods.navigate(context, GroupsPage(model: model,group: model.allGroups[index],)),),
                          ),
                        ),
                        // const SizedBox(height: 10,),
                    ...
                    const CustomRowTitle(title: 'كورسات ومجموعات',),
        ...
        BoxShadow(
          ...
          child: Column(
           ...
           children: [
             ...
             Image.network( course.TeacherPicture.toString(), width: 50, height: 50,),
             Text("كورسات/ " + course.TeacherName!,
           style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.black),
             textAlign: TextAlign.center,
             ),
              Text(course.SubjectName, style: const TextStyle(color: Color(0xFF888B8E)),),
                 ElevatedButton(onPressed: ()=>onStart(), child: const Text('ابدأ الان'))
      ],
    ),
  );
